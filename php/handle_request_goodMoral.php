...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $checkStmt = $conn->prepare("SELECT COUNT(*) FROM good_moral WHERE user_id = ? AND status = 'Ongoing'");
   ...
   $stmt = $conn->prepare("INSERT INTO good_moral (user_id, last_name, first_name, middle_name, suffix, purpose, document_type, supporting_document, status, date_requested) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, NOW())");
    ...
    if ($stmt->execute()) {
        ...
        if($adminRow = $adminQuery->fetch_assoc()) {
            ...
            $notifMsg = "$firstName $lastName (USER ID NO.({$user_id})) have submitted a request for Certificate of Good Moral.";
    ...
    header("Location: ../user/userRequest_goodMoral.php");
    exit();
}
?>
