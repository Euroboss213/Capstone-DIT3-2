...
// Determine actor role from DB (optional but useful if needed)
...
// Update the indigency table
$updateStmt = $conn->prepare("UPDATE indigency SET purpose = ?, supporting_document = ?, reply = ?, status = 'Ongoing' WHERE id = ?");
...
if ($updateStmt->execute()) {
    ...
    // History snapshot
    $fetchStmt = $conn->prepare("SELECT i.*, u.first_name, u.middle_name, u.last_name, u.suffix FROM indigency i JOIN users u ON i.user_id = u.id WHERE i.id = ?");
