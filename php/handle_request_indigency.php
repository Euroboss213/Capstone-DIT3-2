...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($stmt->execute()) {
        ...
        if ($adminRow = $adminQuery->fetch_assoc()) {
            ...
            $notifMsg = "$firstName $lastName submitted an indigency request.";
            $notifStmt = $conn->prepare("INSERT INTO notifications (request_id, actor_id, actor_role, message, sent_to, is_read) VALUES (?, ?, ?, ?, ?, 0)");
            $role = 'user';
            $notifStmt->bind_param("iissi", $request_id, $user_id, $role, $notifMsg, $admin_id);
            $notifStmt->execute();
            $notifStmt->close();
        }
